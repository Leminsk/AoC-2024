...
int dampened_is_safe(std::vector<int> &report) {
	int safety = is_safe(report);
	...
	for(size_t i=0; i<report.size(); ++i) {
		...
		for(size_t j=0; j<report.size(); ++j) {
			if(j != i) {
				one_removed.push_back( report[j] );
			}
		}
		safety = is_safe(one_removed);
		if(safety == 1) { return 1; }
	}
...
int main() {
	...
	for(size_t i=0; i<reports.size(); ++i) {
		safe_amount += dampened_is_safe(reports[i]);
	}
	std::cout << "(dampened) safe_amount: " << safe_amount << '\n';
